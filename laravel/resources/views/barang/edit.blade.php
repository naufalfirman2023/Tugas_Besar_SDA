...
<body>
    @if (session('error'))
        <div class="alert alert-danger">
            <Script>
            swal("Data barang gagal di edit",{button: "OK",icon: "error",});
            </Script>
        </div>
    @elseif (session('success'))
        <Script>
            swal("Data stok barang berhasil di edit", {button: "OK",icon: "success",});
          ...
          <div class="flex flex-wrap-mx-3 mt-2">
            <div class="w-full max-w-full mx-auto mt-0 md:flex-0 shrink-0 md:w-7/12 ">
                ...
                <div class="flex-auto p-6">
                    <form action="/barang/{{ $barang->id }}/restok/update" method="POST">
                        @csrf
                        ...
                        <div class="text-left justify-center">
                            <h5>Nomor sku : {{ $barang->sku }}</h5>
                        </div>
                        <div class="text-left justify-center">
                            <h5>Nama Barang : {{ $barang->nama}}</h5>
                        </div>
                        <div class="text-left justify-center">
                            <h5>Harga Barang : {{ $barang->harga}}</h5>
                        </div>
                        <div class="text-left justify-center">
                            <h5>Stok barang saat ini adalah : {{ $barang->stok }}</h5>
                        </div>
                        <div class="mb-4">
                            <input type="number" placeholder="Stock Tambahan" required name="stok" id="jumlah" class="text-sm focus:shadow-soft-primary-outline leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow" placeholder="Password" aria-label="Password" aria-describedby="password-addon" />
                        </div>
